...
$paramList | ForEach-Object -Parallel {
    ...
    $outFileName = "$datasetName-lsa$improver-$disturb-$solutionConfig.txt"
    $outFilePath = Join-Path $($using:solutionPath) $outFileName
    $ecpArgs = "-f ""$dataset"" -s ""$solution"" -o ""$outFilePath"" -t a -n edge -st $improver -sp1 $runningTime -sp2 $disturb"
